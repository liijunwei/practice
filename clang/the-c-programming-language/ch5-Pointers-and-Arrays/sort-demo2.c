...
修改前面的排序函数, 在给定可选参数-n的情况下,
该函数将按数值大小而非字典顺序对输入进行排序
...
由于排序算法和比较/交换操作无关, 因此, 通过在排序算法中调用不同的比较和交换函数,
便可以实现按照不同的标准排序

TODO ch5-Pointers-and-Arrays/sort-demo2.c:42:83: warning: pointer type mismatch
('int (*)(char *, char *)' and 'int (*)(const char *, const char *)')
...
void custom_qsort(void *lineptr[], int left, int right,
                  int (*comp)(void *, void *));
int numcmp(char *s1, char *s2);

// gcc -g ch5-Pointers-and-Arrays/sort-demo2.c && cat
// ch5-Pointers-and-Arrays/sample-input.md | ./a.out gcc -g
// ch5-Pointers-and-Arrays/sort-demo2.c && cat
// ch5-Pointers-and-Arrays/sample-input.md | ./a.out -n 对输入的文本行进行排序
int main(int argc, char const *argv[]) {
  ...
  if ((nlines = readlines(lineptr, MAXLINES)) >= 0) {
    custom_qsort((void **)lineptr, 0, nlines - 1,
