...
默认情况下, n的值为10, 但可通过一个可选参数改变n的值, 因此, 命令 `tail -n`
将打印其输入的最后n行 无论输入或n的值是否合理, 该程序都应该能正常运行

编写的程序要重复利用存储空间,
输入行的存储方式应该通5.6节中排序程序的存储方式一样, 而不采用固定长度的二维数组
...
// gcc ch5-Pointers-and-Arrays/EX5-13.c && ./a.out <
// ch1-A-Tutorial-Introduction/input.txt gcc ch5-Pointers-and-Arrays/EX5-13.c &&
// ./a.out -4 < ch1-A-Tutorial-Introduction/input.txt gcc
// ch5-Pointers-and-Arrays/EX5-13.c && cat ch1-A-Tutorial-Introduction/input.txt
// | ./a.out -2 print last n lines of the input
