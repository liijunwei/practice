...
/*
  某些情况下, extern 可以省略
  在源文件中, 如果外部变量的定义出现在使用它的函数之前,
  那么在那个函数中就没有必要使用extern声明 在通常做法中,
  所有的外部变量都声明在文件的开始出, 这样就可以省去extern声明

  人们通常把变量和函数的extern声明放在一个单独的文件中(习惯上称为头文件),
  并在每个原文件的开头使用`#include`语句把所有要用的 头文件包含进来
