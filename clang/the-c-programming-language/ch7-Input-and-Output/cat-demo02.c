...
cat程序的功能不完善, 问题在于 如果因某种原因导致其中的一个文件无法访问, 相应的诊断信息要在该连接输出的末尾才能打印出来
当输出到屏幕时, 这种处理方法尚可接受; 但如果输出到一个文件或者通过管道输出到另一个程序时, 就无法接受了(算是脏数据)

为了更好地处理这种情况, 另一个输出流以与stdin和stdout相同的方式分派给程序, 即stderr;
