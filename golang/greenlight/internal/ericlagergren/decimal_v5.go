...
func TryWrapNumericEncodePlan( //nolint: ireturn // ref shopspring
	...
	if fmt.Sprintf("%T", value) == "decimal.Big" {
		return &wrapDecimalEncodePlan{}, Decimal(value.(decimal.Big)), true //nolint:forcetypeassert // ref shopspring
	}
...
func (plan *wrapDecimalEncodePlan) Encode(value interface{}, buf []byte) ([]byte, error) {
	return plan.next.Encode(Decimal(value.(decimal.Big)), buf) //nolint:forcetypeassert,wrapcheck // ref shopspring
}
...
func (plan *wrapDecimalScanPlan) Scan(src []byte, dst interface{}) error {
	return plan.next.Scan(src, (*Decimal)(dst.(*decimal.Big))) //nolint:forcetypeassert,wrapcheck // ref shopspring
}
...
func Register(typeMap *pgtype.Map) {
	typeMap.TryWrapEncodePlanFuncs = append([]pgtype.TryWrapEncodePlanFunc{TryWrapNumericEncodePlan},
	...
	registerDefaultPgTypeVariants := func(name, arrayName string, value interface{}) {
		...
		typeMap.RegisterDefaultPgType(reflect.MakeSlice(sliceOfPointerType, 0, 0).Interface(), arrayName)
