...
+ TODO 问题: "执行算数右移之前加上一个适当的偏置量"是什么意思? (p73)


## 2.3.8 关于整数运算的最后思考

+ 计算机执行的"整数"运算实际上是一种模运算形式;
+ 表示数字的有限字长限制了可能的值的取值范围, 结果可能溢出
...
+ 起初浮点数有多种实现, 后来IEEE制定了浮点数的标准

+ 本节将看到IEEE浮点格式中数字是如何表示的

+ IEEE格式是定义在一组小而一致的原则上的, 所以它实际上是相当优雅而容易理解的

# 2.4.1 二进制小数

+ 就像10进制没法表示 1/3 5/7 这样的数一样, 二进制也没法表示向0.20这样的小数, 只能通过增加二进制位的长度来近似表示
