...
+ C语言的一个不同寻常的特点是 可以产生指向数组重元素的指针, 并对这些指针进行运算; 在机器代码中, 这些运算会被翻译成地址计算

+ 问题: 分不清 C里的 数组指针 和 指针数组, 是不是因为 符号的优先级关系搞不清?
...
+ TODO 问题: 编译器为什么要优化定长数组相关的代码? 转成那种指针形式 有什么好处呢?


## 3.10.1 理解指针(p192 重要)

这里重点介绍一些指针和他们映射到机器代码的关键原则
...
+ `*`操作符用于间接引用指针; 其结果是一个值, 它的类型与该指针的类型一致; 间接引用使用内存引用来实现的, 要么是存储到一个指定的地址, 要么是从指定的地址读取

+ 数组与指针紧密联系

+ 将指针从一种类型强制转成另一种类型, 只改变他们的类型, 不改变他们的值; 强制类型转换的一个效果是 改变指针运算的伸缩(强制类型转换的优先级高于加法)
...
+ TODO 问题: 没看明白 缓冲区溢出的 汇编代码...(p195)

+ "通常, 使用get或者其他任何能导致存储溢出的函数, 都是不好的编程习惯"
